...
for row in calls:
    if row[0][0:5]=='(080)':
        ...
        if row[1][0]=='(':
            Area.add(((row[1].split(')'))[0])[1:4])
        
        if (row[1][0]=='7' or row[1][0]=='8' or row[1][0]=='9'):
            Mobile.add(((row[1].split(' '))[0])[0:4])
        
        if row[1][0:3]=='140':
            Telemarketer.add(row[1])
...
print('The total percentage of the Banglore calls {0:3.2f}%'.format(Percentage))
